...
const Header = ()=>{
    ...
    <>
        <nav className='flex content-between p-1 items-center justify-between md:p-3 border sticky top-0 z-20 w-full shadow-lg rounded-b-lg border-white border-3 backdrop-blur-sm'>
            ...
            <div className={`${isSearching?"flex":"hidden"} border border-black rounded-md active:border-primary focus-within:border-primary p-2 lg:flex lg:w-56 xl:w-auto items-center`}><SearchBar setSearching={setSearching}/></div>
            <div className='flex items-center justify-between'>
                ...
                <ul className={`${isHamOpen?'flex flex-col':'hidden'} absolute top-20 right-8 lg:relative lg:top-0 lg:left-0 lg:flex lg:justify-between lg:content-between lg:space-x-4 lg:h-12`}>
                    ...
                    <li className={`${isSearching || !isHamOpen ? "hidden lg:hidden" : ""} cursor-pointer text-primary h-full bg-black rounded-md my-4 px-2 py-0 flex items-center text-xl lg:my-0 hover:bg-primary hover:text-black hover:transition-all hover:duration-300`}>
                        <div className="mx-auto text-2xl p-2 flex items-center">
                            Cart
                            <div className="text-sm text-black relative bottom-[0.85rem] bg-primary md:p-[0.07rem] rounded-full font-bold">27</div>
                        </div>
                    </li>
                </Link>}
                    <Link to='/contact'><li onClick={() =>{setHamOpen(false)}} className='cursor-pointer text-primary h-full bg-black rounded-md my-4 px-2 py-0 flex items-center text-xl lg:ml-0 lg:my-0 hover:bg-primary hover:text-black hover:transition-all hover:duration-300'><div className="mx-auto text-2xl p-1">Contact</div></li></Link>
                </ul>
                <Link to='/cart'>
